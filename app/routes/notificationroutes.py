    ...
    def notifications(self):
        ...
        def index():
            ...
            return render_template(
                ...
                unread_count=NotificationModel.unread_count(user_id),
            )
        ...
        def recent():
            ...
            user_id = session['user_id']
            notifications = NotificationModel.get_notifications(user_id)[:10]
            serialized = [
                {
        ...
        def mark_read(notification_id):
            ...
            updated = NotificationModel.mark_read(notification_id, session['user_id'])
        ...
        def open_notification(notification_id):
            if 'user_id' not in session:
                return redirect(url_for('login.index'))
            user_id = session['user_id']
            notification = NotificationModel.get_notification(notification_id, user_id)
            if not notification:
                return redirect(url_for('notifications.index'))

            NotificationModel.mark_read(notification_id, user_id)

            if notification.get('Type') == 'new_message' and notification.get('Reference_id'):
                message = MessageController.get_message(notification['Reference_id'])
        ...
        def mark_all_read():
            if 'user_id' not in session:
                return jsonify({'status': 'error', 'message': 'Unauthorized'}), 401
            updated = NotificationModel.mark_all_read(session['user_id'])
        ...
        def unread_count():
            ...
            return jsonify({'count': NotificationModel.unread_count(session['user_id'])})
